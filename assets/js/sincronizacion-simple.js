...
async function cargarHistorialFirebase() {
  ...
  try {
    ...
    const [historialSnapshot, sentimientosSnapshot] = await Promise.all([
      db.collection('historial').get(),
      db.collection('sentimientos').get()
    ]);

    const historialCompleto = [];

    // Agregar elementos del historial general
    ...
    sentimientosSnapshot.forEach(doc => {
      const data = doc.data();
      historialCompleto.push({
        ...data,
        firebaseId: doc.id,
    ...
    // Agregar historial local como fallback
    const historialLocal = JSON.parse(localStorage.getItem('historial-tareas') || '[]');
    historialCompleto.push(...historialLocal);

    // Ordenar por timestamp/fecha
    historialCompleto.sort((a, b) => {
      const fechaA = new Date(a.timestamp || a.fecha + 'T00:00:00');
      const fechaB = new Date(b.timestamp || b.fecha + 'T00:00:00');
